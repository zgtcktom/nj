...
function reduce_body(
  ...
  let x_start_decl = nargs.map(
    i =>
      `let ${x_start[i]} = ${x_offset[i]}${
        outer_axis.length ? " + " : ""
      }${outer_axis.map(j => `${index[j]} * ${x_strides[i][j]}`).join(" + ")};`
  );

  let inner_loop_x = nargs.map(
    i =>
      `${args[i]}.data[${x_start[i]}${
        inner_axis.length ? " + " : ""
      }${inner_axis.map(j => `${index[j]} * ${x_strides[i][j]}`).join(" + ")}]`
  );

  let outer_loop_out = `${out}.data[${offset}${
    outer_axis.length ? " + " : ""
  }${outer_axis.map((i, j) => `${index[i]} * ${strides[j]}`).join(" + ")}]`;

  let inner_loop_body = `${inner_loop.join(
    " "
  )} {${accum} = ${func}(${accum}, ${inner_loop_x.join(", ")});}`;
  ...
  if (strides.length > 0)
    decl += `let [${strides.join(", ")}] = ${out}.strides;`;
  decl += `let [${shape.join(", ")}] = ${args[0]}.shape;`;
  decl += nargs
    .map(i => `let [${x_strides[i].join(", ")}] = ${args[i]}.strides;`)
    .join("");
  decl += nargs.map(i => `let ${x_offset[i]} = ${args[i]}.offset;`).join("");

  let loop = outer_loop.length
    ? `${outer_loop.join(" ")} {${outer_loop_body}}`
    : outer_loop_body;
...
function func_body(narg, ndim) {
  ...
  let strides = [...Array(narg).keys()].map(i =>
    [...Array(ndim).keys()].map(j => `strides${i + 1}_${j}`)
  );
  let shapes = [...Array(narg).keys()].map(i =>
    [...Array(ndim).keys()].map(j => `shape${i + 1}_${j}`)
  );
...
function wrapper_map(builder, self = Object.create(null)) {
  ...
  return function (builder, narg, ndim) {
    return (
      (this[narg] ?? (this[narg] = {}))[ndim] ??
      (this[narg][ndim] = builder(narg, ndim))
    );
  }.bind(self, builder);
}
...
const map_func = wrapper_map((narg, ndim) => {
  let func = "fn";
  let { declare, out, args, loop, outoffset, strides, offsets } = func_body(
    narg,
    ndim
  );
...
function binary_tree(tree, ndim, path, builder, narg, axis) {
  ...
  return (
    tree[!!path[ndim - 1]] ??
    (tree[!!path[ndim - 1]] = builder(narg, ndim, axis))
  );
}
...
function cached_reduce(builder, self = Object.create(null)) {
  ...
  return function (builder, narg, ndim, axis) {
    let path = {};
    if (axis != null)
      for (let i = 0; i < axis.length; i++) path[axis[i]] = true;
    return binary_tree(
...
let destructuringObject = (
  ...
  for (let i = 0; i < keys.length; i++) {
    pair.push(
      keys[i] == variables[i] ? `${keys[i]}` : `${keys[i]}: ${variables[i]}`
    );
  }
  if (declaration != null) {
    return `${declaration} {${pair.push(", ")}} = ${object}`;
  }
  return `({${pair.push(", ")}} = ${object})`;
};
...
function declarations(obj, data, offset, strides) {
  ...
  statements.push(
    destructuringArray("let", strides, dotAccess(obj, "strides"))
  );
...
function _map_body(narg, ndim) {
  ...
  statements.push(destructuringArray("let", shape, dotAccess(x[0], "shape")));
          ...
          assignment(
            ...
            [
              bracketAccess(
                ...
                sum(
                  out_offset,
                  ...index.map((index, axis) =>
                    product(index, out_strides[axis])
                  )
                )
              ),
            ],
                ...
                ...x_data.map((x_data, i) =>
                  bracketAccess(
                    ...
                    sum(
                      x_offset[i],
                      ...index.map((index, axis) =>
                        product(index, x_strides[i][axis])
                      )
                    )
                  )
                )
                      ...
                      bracketAccess(
                        ...
                        sum(
                          ...
                          ...range(ndim)
                            .filter(i => axis[i])
                            .map(axis =>
                              product(index[axis], x_strides[i][axis])
                            )
                        )
                      )
                    )
...
export function _wrap_map(name, fn, narg = fn.length) {
  let func;
  if (narg == 1)
    func = (x1, out = null) => {
      x1 = asarray(x1);
...
export function _wrap_reduce(name, fn, narg = fn.length - 1, defaultinitial) {
  ...
  if (narg == 1)
    func = (x1, axis = null, out = null, initial = defaultinitial) => {
      ...
      let shape;
      if (axis == null) {
        axis = Array(x1.ndim).fill(1);
        shape = [];
      } else {
        axis = normalize_axis_mask(axis, x1.ndim);
        shape = [];
        for (let i = 0; i < x1.ndim; i++) if (!axis[i]) shape.push(x1.shape[i]);
      }
      if (out == null) out = empty(shape);
      else if (!shallow_array_equals(shape, out.shape)) throw "unmatch shape";

      return _reduce(1, x1.ndim, axis)(fn, x1, out, initial);
    };
...
function anonymous(fn, x1, out, initial) {
  ...
  for (let i_0 = 0; i_0 < shape_0; i_0++) {
    let accum = initial;
    let x1_start = x1_offset + i_0 * x1_strides_0;
    accum = fn(accum, x1_data[x1_start]);
    out_data[out_offset + i_0 * out_strides_0] = accum;
  }
  return out;
}
