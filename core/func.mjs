...
function variableNames(narg, ndim, out_dim = ndim) {
	...
	return { fn, out, x, out_offset, x_offset, out_data, x_data, out_strides, x_strides, index, shape };
}
...
function declarations(obj, data, offset, strides) {
	...
	statements.push(destructuringArray('let', strides, dotAccess(obj, 'strides')));

	return statements;
}

function _map_body(narg, ndim) {
	let { fn, out, x, out_offset, x_offset, out_data, x_data, out_strides, x_strides, index, shape } = variableNames(
		narg,
		ndim
	);
...
export function _wrap_reduce(name, fn, narg = fn.length - 1, defaultinitial) {
	...
	if (narg == 1)
		func = (x1, axis = null, out = null, initial = defaultinitial) => {
			...
			return _reduce(1, out.ndim, axis)(fn, x1, out, initial);
		};
